...
// for VCR cassettes: the host now owns the provider call, so the plugin's tests
// need only a deterministic host.
...
// A producer rule fires llm_review; consumer rules react to each verdict. This
// is the shape llm-review.md describes.
const llmRuleset = `import "talooner.tln"

rule "Review against module docs" {
  for records where type == "pr" and attr "pr.needs_review" == true
  do llm_review "https://docs/module"
}
...
// baseLLMArgs returns a fresh args map for a PR that needs review at sha1.
func baseLLMArgs() map[string]string {
	return map[string]string{
		...
		"head_sha": "sha1",
		"ruleset":  llmRuleset,
		"facts":    `{"pr.needs_review": true, "pr.mergeable": true}`,
	}
}
...
// The two-pass path: the producer fires llm_review, its result is asserted as a
// fact, and the consumer reaches its verdict on the second pass — with exactly
// one model call and no llm_review verb in the returned actions.
func TestLLMReviewTwoPassBlocksOnMismatch(t *testing.T) {
	host := &fakeHost{verdict: "mismatch", explain: "contradicts docs"}
	resp := evaluateHosted(New(), host, baseLLMArgs())
	got := decodeEval(t, resp)

	if !hasBlock(got.Actions, "pr.merge") {
		t.Fatalf("consumer rule should block on mismatch (second pass), got %+v", got.Actions)
	}
	if host.count() != 1 {
		t.Errorf("want exactly one model call, got %d", host.count())
	}
	// llm_review is never returned to the bot; there is no proto verb for it, so
	// its presence would show up as an unknown/unspecified verb.
	for _, a := range got.Actions {
		if a.Verb == taloonerpb.Verb_VERB_UNSPECIFIED {
			t.Errorf("llm_review must not appear in returned actions, got %+v", a)
		}
	}
}
...
func TestLLMReviewCachedAtSameSHA(t *testing.T) {
	...
	resp := evaluateHosted(s, host, baseLLMArgs())
	got := decodeEval(t, resp)
...
// Quota exhausted: no model call is made, the verdict is error, and the ruleset's
// error branch fires — nothing is silently approved.
...
// A real call decrements the tenant's live quota, and whoami surfaces it.
...
// Standalone (no host): a fired llm_review degrades to error rather than
// panicking on a nil host, and whoami withdraws the feature.
func TestLLMReviewStandaloneDegrades(t *testing.T) {
	s := New()
	s.SetStandalone(true)

	// Unary Execute passes a nil host — the standalone path.
	resp := s.Execute(plugin.Request{ID: "e", Action: "evaluate_pr", Args: baseLLMArgs()})
	got := decodeEval(t, resp)
	if hasBlock(got.Actions, "pr.merge") {
		t.Error("without a host there is no verdict; the mismatch branch must not fire")
	}
	if !hasCommentContaining(got.Actions, "could not run") {
		t.Errorf("a host-less llm_review should land on the error branch, got %+v", got.Actions)
	}
}
...
func TestWhoamiWithdrawsFeatureWhenStandalone(t *testing.T) {
	s := configured(t)
	s.SetStandalone(true)
	resp := whoamiCall(s, map[string]string{auth.ArgAPIKey: "sekret"})
	if resp.Error != "" {
		t.Fatalf("whoami errored: %s", resp.Error)
	}
	tenant, _ := s.auth.Authenticate("sekret")
	if tenant.HasFeature("llm_review") == false {
