...
// build, so the shipped safety policy stays verified. The talon CLI is the
...
func TestStrictBaseSuite(t *testing.T) {
	cmd := exec.Command("go", "tool",
		"github.com/opentalon/talon-language/cmd/talon", "test",
		"base/talooner.tln", "base/talooner.tln.test")
	out, err := cmd.CombinedOutput()
	t.Logf("talon test output:\n%s", out)
	if err != nil {
		t.Fatalf("talon test failed: %v", err)
	}
