...
(`talon-language/internal/defeasible/defeasible.go:33`); `overrides` across two
...
Since [`talon-language#159`](https://github.com/opentalon/talon-language/issues/159)
...
**`talon-language` does not validate verb names.** The verb vocabulary belongs
...
- **`deploy_preview` / `screenshot` / `scan_dependencies` are not verbs.** They
  parse — `do deploy_preview "pr"` is valid Talon — so `validate_ruleset`
